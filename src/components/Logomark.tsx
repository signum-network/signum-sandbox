 ...
 * The Signum logomark.
 *
 * Taken from `icon.svg`, which is the favicon, with one difference that
 * matters: the favicon paints a dark disc behind the mark, because the sigil
 * is cut *out* of the disc and would vanish against a pale browser tab. Here
 * there is nothing to guard against — the page shows through the knockout,
 * which is how the mark is meant to read — and dropping the backdrop lets the
 * whole thing take `currentColor` and follow the theme.
 */
export function Logomark({
   ...
   *
   * The key makes the value re-run the animation: a one-shot on an element
   * with no other state has nothing to interpolate from, and remounting a
   * static path costs nothing.
   */
  beat?: number
}) {
  return (
